<x-app title="Editar Obra">
    <div class="bg-dark pt-3 px-3 position-sticky sticky-top">
        <h4><b>EDITAR OBRA:</b></h4>
        <hr class="text-primary">
    </div>
    <form action="{{ route('works.update', $work->id)}}" method="POST" enctype="multipart/form-data" class="p-3 ">
        @csrf
        @method('PUT')
        <div class="form-group">
            <div class="form-group col">
                <label for="name">Nome:</label>
                <input class="form-control input-dark" type="text" id="name" name="name" required value="{{$work->name}}">
            </div>
        </div>
        <div class="form-group mt-3">
            <label for="synopsis">Sinopse:</label>
            <textarea class="form-control input-dark" name="synopsis" id="synopsis" cols="30" rows="10" required value="">
                {{$work->synopsis}}
            </textarea>
        </div>
        <div class="form-group mt-3">
            <label for="release_date">Data de Lancamento:</label>
            <input class="form-control input-dark" type="date" name="release_date" id="release_date" required value="{{$work->release_date}}"">
        </div>
        <div class="form-row gap-2 d-flex justify-content-between mt-3">
            <div class="form-group col col>
                    <label for="chapters">Capitulos/Episodios:</label>
            <input class="form-control input-dark" type="number" name="chapters" id="chapters" required value="{{$work->chapters}}">
        </div>
        <div class="form-group col col">
            <label for="volumes">Volumes/Temporadas:</label>
            <input class="form-control input-dark" type="number" name="volumes" id="volumes" required value="{{$work->volumes}}">
        </div>
        </div>
        <div class="form-row gap-2 d-flex mt-3">
            <div class="form-group col">
                <label for="producer">Produtora/Editora:</label>
                <input class="form-control input-dark" type="text" name="producer" id="producer" required value="{{$work->producer}}">
            </div>
            <div class="form-group col">
                <label for="author">Autor:</label>
                <input class="form-control input-dark" type="text" name="author" id="author" required value="{{$work->author}}">
            </div>
            <div class="form-group col">
                <label for="status">Status: </label>
                <select class="form-control input-dark" name="status" id="status" required>
                    <option class="bg-secondary" @if($work->status == 0) selected @endif value="0">Em Andamento</option>
                    <option class="bg-secondary" @if($work->status == 1) selected @endif value="1">Em Hiato</option>
                    <option class="bg-secondary" @if($work->status == 2) selected @endif value="2">Finalizado</option>
                </select>
            </div>
        </div>
        <div class="form-row gap-2 d-flex mt-3">
            <div class="form-group col">
                ...
                <input class="form-control input-dark" type="text" name="image" id="image" required value="{{$work->image}}">
            </div>
            <div class="form-group col">
                <label for="type">Tipo da Obra:</label>
                <select class="form-control input-dark" name="type" id="type" required>
                    <option class="bg-secondary"  @if($work->type == 0) selected @endif value="0">Anime</option>
                    <option class="bg-secondary" @if($work->type == 1) selected @endif value="1">Manga</option>
                </select>
            </div>
        </div>
            ...
            <div class="input-dark rounded-3">
                ...
                <div id="addWorkCharacters" class="p-2"  style="overflow: auto; height: 200px">
                    @foreach ($characters as $character)
                        ...
                        @foreach($work->characters as $wcharacter)
                            @if($wcharacter->id == $character->id)
                                checked
                            @endif
                            @endforeach>
